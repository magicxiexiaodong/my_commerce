package com.xxd.mock

/*
 * Copyright (c) 2018. Xxd Inc. All Rights Reserved.
 */

import java.util.UUID

import com.xxd.commos.model.{ProductInfo, UserInfo, UserVisitAction}
import com.xxd.commos.utils.{DateUtils, StringUtils}
...
object MockDataGenerate {
  ...
  private def mockUserVisitActionData(): Array[UserVisitAction] = {
    ...
    for (i <- 0 to 100) {
      ...
      // 每个用户产生10个session
      for (j <- 0 to 10) {
        // 不可变的，全局的，独一无二的128bit长度的标识符，用于标识一个s ession，体现一次会话产生的sessionId是独一无二的
